...
FFGraphFormat* ffgraphfmt_init(SourceHandle* sourcehandle, const enum AVMediaType required_type) {
    ...
    ffgraphfmt->iohandle = iohandler_init(sourcehandle);
    ffgraphfmt->codec_ctx = avcodec_alloc_context3(NULL);
    ffgraphfmt->av_frame = av_frame_alloc();
    ffgraphfmt->has_ended = false;
    ffgraphfmt->has_decoded = false;
    memset(&ffgraphfmt->ffgraphconv, 0x00, sizeof(FFGraphConversor));
...
#ifdef DEBUG
            printf("ffgraphfmt_read_and_seek() call to avcodec_receive_frame() failed, reason: %s\n", av_err2str(ret));
#endif
        }
