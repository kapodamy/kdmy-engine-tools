...
#define SAMPLE_TYPEDEF int16_t
#define SAMPLE_SIZE sizeof(SAMPLE_TYPEDEF)

#define BUFFER_SAMPLE_LENGTH 8192
#define BUFFER_BYTE_LENGTH (BUFFER_SAMPLE_LENGTH * SAMPLE_SIZE)

static bool custom_output_enable = false;
static int custom_output_rate = -1;
static int custom_output_channels = -1;
static bool custom_output_channels_only_downmix = true;

typedef struct {
    struct SwrContext* swr_ctx;
    SAMPLE_TYPEDEF* sample_buffer;
...
bool audioconverter_init(const AVCodecContext* codec_ctx, FFGraphConversor* ffgraphconv) {
    ...
    if (sample_rate > 48000) sample_rate = SAMPLE_RATE;

    if (custom_output_enable) {
        if (custom_output_channels > 0) {
            if (custom_output_channels_only_downmix) {
                if (channel_count > custom_output_channels) channel_count = custom_output_channels;
            } else {
                channel_count = custom_output_channels;
            }
        }
        if (custom_output_rate > 0) sample_rate = custom_output_rate;
    }
    ...
    obj->swr_ctx = swr_ctx;
    obj->sample_buffer = av_malloc(BUFFER_BYTE_LENGTH * channel_count);
    obj->sample_buffer_used = 0;
...
#ifdef DEBUG
    memset(obj->sample_buffer, 0x00, BUFFER_BYTE_LENGTH);
#endif
...
void audioconverter_enable_custom_output(bool enable_custom, int rate, int channels, bool downmix_only) {
    ...
    custom_output_channels = channels;
    custom_output_channels_only_downmix = downmix_only;
}
